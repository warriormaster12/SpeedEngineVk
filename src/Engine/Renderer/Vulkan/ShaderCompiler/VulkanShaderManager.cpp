#include "Engine/Renderer/Vulkan/ShaderCompiler/VulkanShaderManager.h"
...
namespace Renderer
{
    ...
    std::string VulkanShaderManager::GetSuffix(const std::string& name)
    {
        const size_t pos = name.rfind('.');
        return (pos == std::string::npos) ? "" : name.substr(name.rfind('.') + 1);
    }
    EShLanguage VulkanShaderManager::GetShaderStage(const std::string& stage)
    {
        if (stage == "vert") {
    ...
    const std::vector<unsigned int> VulkanShaderManager::CompileGLSL(const std::string& filename)
    {
    ...
    std::string VulkanShaderManager::GetFilePath(const std::string& str)
    {
     ...
     VkShaderModule VulkanShaderManager::createShaderModule(const std::vector<char>& code, VkDevice& device) {
