...
namespace Renderer
{
    ...
    std::vector <VkBuffer> VulkanUniformBuffer::createBuffer(std::vector <VkBuffer> inputBuffer, std::vector <VmaAllocation> inputAllocation, uint32_t byte_size, uint32_t size)
    {
        ...
        inputAllocation.resize(size);


        for (size_t i = 0; i < size; i++) {
            vmaCreateBuffer(p_vulkanMemoryAllocator->allocator, &UniformBufferInfo, &allocInfo, &outputBuffer[i], &inputAllocation[i], nullptr);
        }
        return outputBuffer;
    }

    void VulkanUniformBuffer::updateBuffer(double deltaTime)
    {
