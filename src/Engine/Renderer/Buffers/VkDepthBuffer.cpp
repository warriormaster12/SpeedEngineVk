...
namespace VkRenderer
{
    void VkDepthBuffer::createDepthResources(VkExtent2D& swapChainExtent)
    {
        ...
        mesh_ref->texture_m_ref.createImage(swapChainExtent.width, swapChainExtent.height, 1, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
        depthImageView = mesh_ref->texture_m_ref.createImageView(depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);
    }
