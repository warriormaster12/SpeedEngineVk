...
namespace Renderer
{
    void Mesh::initMesh(Vulkan& vulkan_api)
    {
        ...
        p_vulkan_api->vulkanObjectBuffers.vulkanDescriptors.createDescriptorSets(p_vulkan_api->vulkanObjectBuffers.vulkanUniformBuffer.createBuffer(uniformBuffers, uboAllocation, sizeof(UniformBufferObject), p_vulkan_api->vulkanImages.vulkanSwapChain.swapChainImages.size()), sizeof(UniformBufferObject));
        ...
        std::cout<<p_vulkan_api->vulkanImages.vulkanSwapChain.swapChainImages.size()<<std::endl;
    }
    ...
    void Mesh::drawMesh()
    {
        ...
        vmaMapMemory(p_vulkan_api->vulkanMemoryAllocator.allocator, uboAllocation[p_vulkan_api->imageIndex], &data);
            memcpy(data, &ubo, sizeof(ubo));
        vmaUnmapMemory(p_vulkan_api->vulkanMemoryAllocator.allocator, uboAllocation[p_vulkan_api->imageIndex]);
    }
}